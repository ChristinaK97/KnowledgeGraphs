...
def getSeparators(header, ninjaHeader):
    seps = []
    sep = ''
...
def repairSingleChar(ninjaHeader, seps):
    upNinjaHeader = []
    rearUpdated = False
...
separators = [getSeparators(h, nh) for h, nh in zip(headers, ninjaHeaders)]
upNinjaHeaders = [repairSingleChar(nh, seps) for nh, seps in zip(ninjaHeaders, separators)]
